...
class UserFriendshipTest < ActiveSupport::TestCase
  ...
  test "that creating a friendship baed on user id and friend id created a pending friendship" do
  ...
  context "#mutual_friendship" do
    setup do
      ...
      @friendship1 = users(:jason).user_friendships.where(friend_id: users(:jim)).first
      @friendship2 = users(:jim).user_friendships.where(friend_id: users(:jason)).first
    end
